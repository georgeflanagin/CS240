...
function vdiff
{
    difffile=$(mktemp)
    if [ -z $1 ]; then
        echo "Diffing checked-in files with current files." > $difffile
        git diff --patience >> $difffile
    elif [ -z "$2" ]; then
        echo "Diffing $1 with current branch" > $difffile
        git diff --patience "$1" >> $difffile
    else
        echo "Diffing $2 and $1" > $difffile
        git diff --patience "$1" "$2" >> $difffile
    fi
    minlines=3
    lines=`wc -l < $difffile`
...
{
    if [ -z $1 ]; then
        echo 'Usage: tombstone {something else you want to do}'
        return 1
    fi
    now=`date +%F-%H-%M`
    out="${1##*/}.$now.log"
...
tools=(awk sed grep cut touch sort git head tail $less tput)
echo -n "Found: "

for t in ${tools[@]}; do
  if [ -z $(which $t 2>/dev/null) ]; then
    echo "WARNING: You will need to install $t for these commands to work."
    return
  else
    echo -n "$t "
  fi
...
echo "Step $(( step++ )): ""This machine is $HOSTNAME"
...
function prompter
{
  ...
  if [ $((promptsize * 5)) -gt $((colwidth * 2))  ]; then
...
function directedmerge
{
    if [ -z $2 ]; then
        echo "Usage directedmerge {from-branch} {onto-branch}"
        echo " *** NOTE: neither branch needs to be 'this' branch. ***"
        return
    fi

    workingbranch=$(git branch | grep \* | awk '{print $2}')
    git commit -a
    git checkout "$2"
    git merge --no-ff -s recursive -X patience "$1"
...
function merge
{
  ...
  if [ -z $1 ]; then
    echo "Merging current branch to master"
    workingbranch=$(git branch | grep \* | awk '{print $2}')
  else
    workingbranch=$1
  fi
  if [ "$workingbranch" == "master" ]; then
    echo "Hey, you are already on master!"
    return
  fi

  git checkout master
  git merge --no-ff -s recursive -X patience "$workingbranch"
  git checkout "$workingbranch"
}
...
function combine
{
    if [ -z $1 ]; then
        echo '  Usage:'
        echo '    combine better_branch onto worse_branch'
        echo '   '
        echo '    This function accepts all the changes that are on better_branch'
        echo '    and makes them a part of worse_branch by overwriting any'
        echo '    conflicts on worse_branch. If you leave off worse_branch, then combine'
        echo '    assumes the target is the current branch.'
        echo '   '
        echo '  Examples:'
        echo '    combine new-fname onto master'
        echo '      effect -> master will now have all the changes that were in new-fname,'
        echo '       even if master had some conflicting code.'
        echo '    combine master'
        echo '      effect -> the current branch will now have all the changes that'
        echo '        are in master.'
        echo '   '
        return
    fi
    ...
    if [ ! $? ]; then
        echo 'You have uncommitted changes. Commit them or stash them.'
        return
    fi
    ...
    better_branch="$1"
    if [ -z "$3" ]; then
        worse_branch="$current_branch"
    else
        worse_branch="$3"
    fi

    git checkout --quiet "$better_branch"
    if [ ! $? ]; then
        git checkout "$current_branch"
        echo "Unknown branch $better_branch."
        return
    fi

    git merge --quiet --strategy=recursive -X ours "$worse_branch"
    if [ ! $? ]; then
        git checkout "$current_commit"
        echo "Unknown branch $worse_branch"
        return
    fi
    ...
    echo "***********************************************************************"
    echo "  Success!!! You have now combined $better_branch onto $worse_branch. They"
    echo "  are now identical."
    echo "***********************************************************************"
    git checkout "$current_branch"
...
echo " "
export PROMPT_COLOR=$LIGHT_PURPLE
